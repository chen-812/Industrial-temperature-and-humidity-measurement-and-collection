...
/****************************************************************************
*    ����ԭ��: IICtoGP_T_SendDATA(void);
*    ��    ��: ��GP8302--T����12bit��DATA
****************************************************************************/
void IICtoGP_T_SendDATA(uint16_t DATA)
{
    unsigned char   data_frame1 = 0xB0; //TIOUT ��ַ ��011  A3
     unsigned char    data_frame2 = 0x02;
     unsigned char    data_low;
     unsigned char    data_high;

    data_low  = (DATA<<4); //�õ�0xD3 D2 D1 D0 0 0 0 0
    data_high = (DATA>>4); //����D11 D10 D9 D8 D7 D6 D5 D4

    iic_start();             //GP_H������ʼ
    iic_send_byte(data_frame1);
    iic_ack();           //GP_H�ȴ���ȡУ����ACK
    iic_send_byte(data_frame2);
    iic_ack();           //GP_H�ȴ���ȡУ����ACK
